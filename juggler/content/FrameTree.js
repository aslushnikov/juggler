...
class FrameTree {
  constructor(rootBrowsingContext) {
    ...
    this._eventListeners = [
      ...
      helper.addObserver((subject, topic, eventInfo) => {
        const [type, jugglerId] = eventInfo.split(' ');
        this.emit(FrameTree.Events.InputEvent, { type, jugglerId: +(jugglerId || '0') });
  ...
  onWindowEvent(event) {
    ...
    if (frame === this._mainFrame) {
      ...
      const emitInputEvent = (event) => this.emit(FrameTree.Events.InputEvent, { type: event.type, jugglerId: 0 });
      this._dragEventListeners = [
        ...
        helper.addEventListener(chromeEventHandler, 'dragend', emitInputEvent, options),
      ];
    }
  }
