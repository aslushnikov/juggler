...
class PageTarget {
  ...
  updateOverridesForBrowsingContext(opt_browsingContext) {
    this.updateTouchOverride(opt_browsingContext);
    this.updateUserAgent(opt_browsingContext);
    this.updatePlatform(opt_browsingContext);
    this.updateDPPXOverride(opt_browsingContext);
    this.updateEmulatedMedia(opt_browsingContext);
    this.updateColorSchemeOverride(opt_browsingContext);
    this.updateReducedMotionOverride(opt_browsingContext);
    this.updateForcedColorsOverride(opt_browsingContext);
  }

  updateTouchOverride(opt_browsingContext) {
    (opt_browsingContext || this._linkedBrowser.browsingContext).touchEventsOverride = this._browserContext.touchOverride ? 'enabled' : 'none';
  }

  updateUserAgent(opt_browsingContext) {
    (opt_browsingContext || this._linkedBrowser.browsingContext).customUserAgent = this._browserContext.defaultUserAgent;
  }

  updatePlatform(opt_browsingContext) {
    (opt_browsingContext || this._linkedBrowser.browsingContext).customPlatform = this._browserContext.defaultPlatform;
  }

  updateDPPXOverride(opt_browsingContext) {
    (opt_browsingContext || this._linkedBrowser.browsingContext).overrideDPPX = this._browserContext.deviceScaleFactor || this._initialDPPX;
  }
  ...
  updateEmulatedMedia(opt_browsingContext) {
    (opt_browsingContext || this._linkedBrowser.browsingContext).mediumOverride = this.mediumOverride;
  }
  ...
  updateColorSchemeOverride(opt_browsingContext) {
    (opt_browsingContext || this._linkedBrowser.browsingContext).prefersColorSchemeOverride = this.colorScheme || this._browserContext.colorScheme || 'none';
  }
  ...
  updateReducedMotionOverride(opt_browsingContext) {
    (opt_browsingContext || this._linkedBrowser.browsingContext).prefersReducedMotionOverride = this.reducedMotion || this._browserContext.reducedMotion || 'none';
  }
  ...
  updateForcedColorsOverride(opt_browsingContext) {
    (opt_browsingContext || this._linkedBrowser.browsingContext).forcedColorsOverride = (this.forcedColors !== 'no-override' ? this.forcedColors : this._browserContext.forcedColors) || 'no-override';
  }
